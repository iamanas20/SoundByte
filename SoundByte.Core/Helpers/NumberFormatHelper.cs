...
namespace SoundByte.Core.Helpers
{
    ...
    public static class NumberFormatHelper
    {
        ...
        public static string FormatTimeString(double inputMilliseconds)
        {
            ...
            return timeSpan.TotalHours < 1.0 ? string.Format("{0:D2}:{1:D2}", timeSpan.Minutes, timeSpan.Seconds) : string.Format("{0:D2}:{1:D2}:{2:D2}", (int)timeSpan.TotalHours, timeSpan.Minutes, timeSpan.Seconds);
        }
        ...
        public static string GetTimeDateString(DateTime date, bool useShortName)
        {
            ...
            var formattedTime = date.DayOfYear == currentDateTime.DayOfYear || date.DayOfYear == currentDateTime.DayOfYear - 1 ? string.Format("{0:t}", date) : string.Empty;
            // Check if yesterday
            if (date.DayOfYear == currentDateTime.DayOfYear - 1)
            {
                return useShortName ? resources.GetString("GeneralText_Yesterday") : string.Format("{0} - {1}", resources.GetString("GeneralText_Yesterday"), formattedTime);
            }

            if (date.DayOfYear == currentDateTime.DayOfYear)
            {
                return useShortName ? formattedTime : string.Format("{0} - {1}", resources.GetString("GeneralText_Today"), formattedTime);
            }
