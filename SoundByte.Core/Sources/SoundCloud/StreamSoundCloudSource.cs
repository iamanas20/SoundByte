...
{
    [UsedImplicitly]
    public class StreamSoundCloudSource : ISource<GroupedItem>
    {
        public async Task<SourceResponse<GroupedItem>> GetItemsAsync(int count, string token, CancellationTokenSource cancellationToken = null)
        {
            if (SoundByteV3Service.Current.IsServiceConnected(ServiceType.SoundCloud))
            {
                ...
                var items = await SoundByteV3Service.Current.GetAsync<StreamTrackHolder>(ServiceType.SoundCloud, "/e1/me/stream",
