    ...
    public static class NumberFormatHelper
    {
        ...
        public static string GetTimeDateString(DateTime date, bool useShortName)
        {
            ...
            if (date.DayOfYear == currentDateTime.DayOfYear - 1)
                return useShortName
                    ? resources.GetString("GeneralText_Yesterday")
                    : string.Format("{0} - {1}", resources.GetString("GeneralText_Yesterday"), formattedTime);

            if (date.DayOfYear == currentDateTime.DayOfYear)
                return useShortName
                    ? formattedTime
                    : string.Format("{0} - {1}", resources.GetString("GeneralText_Today"), formattedTime);
